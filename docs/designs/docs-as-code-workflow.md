...
**Version:** 1.0.0
**Status:** Accepted
**Scope:** `fer-osorio/claude-sandbox`
...
Design documentation has been produced on an ad-hoc basis: one detailed design document exists for the global layer injection feature, written during implementation. There is no standing policy for when to write a document, what format it must follow, or how documents connect to commits and GitHub Issues. No architecture decision records exist. There is no `BUILDING.md` or `ARCHITECTURE.md`. Operational knowledge — Squid proxy configuration, image build procedures, agent definitions — lives in `docs/` files without a consistent naming or format convention.
...
### Case E — Container security control change

**Applies to:** Any modification to the files that directly govern what a
container process can do, read, or write: `base/Dockerfile`,
`base/entrypoint.sh`, `squid/squid.conf`, or the `permissions` block in
`settings.json`.

Security-affecting changes carry an additional obligation: the impact on
the STRIDE threat model must be documented before the implementation is
merged. This applies regardless of whether the change also qualifies as
Case B, C, or D — the STRIDE analysis is additive.
...
3. The STRIDE analysis must cover:
   ...
   - What controls were added or removed.
   - Residual risk after controls are applied.

**Artifacts produced:** GitHub Issue; updated design document or security
plan section; ADR if the decision is architectural.
...
```
0. Does this change touch container security controls
   (base/Dockerfile, base/entrypoint.sh, squid/squid.conf,
   or the permissions block in settings.json)?
...
| Container security control change | E | Issue + STRIDE analysis (design doc or security plan amendment) |
...
**Container security control:** Any file that directly governs what a
container process can do, read, or write: `base/Dockerfile`,
`base/entrypoint.sh`, `squid/squid.conf`, or the `permissions` block in
`settings.json`.
